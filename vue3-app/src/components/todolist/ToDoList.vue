...
export default {
    ...
    methods: {
        ...
        handleCompare(option, order) {
            return (a, b) => {
                if(!this.hasProperty(a, option) || !this.hasProperty(b, option)) {
        ...
        handleEditTask(id, name, level) {
            // Copy current list
            let currentList = [...this.list];

            // Find index of edited task in list
            let taskIndex = currentList.findIndex(item => {
                return item.id === id
            })
        ...
        handleRemoveTask(id) {
            ...
            let updatedList = currentList.filter(item => {
                return item.id !== id;
            });
    ...
    mounted() {
        ...
        if(this.hasProperty(localStorage, 'newList')) {
        ...
        if (this.hasProperty(localStorage, 'editedList')) {
        ...
        if (this.hasProperty(localStorage, 'updatedList')) {
